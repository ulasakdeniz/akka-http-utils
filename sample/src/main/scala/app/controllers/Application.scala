...
object Application extends Controller with StrictLogging {
    ...
    get {
      pathSingleSlash {
        ...
        path("user" / Remaining) { userName =>
          {
            val eventualResponse = getUserTwitterData(userName).map(byteStringOpt => {
              byteStringOpt
                .map(byteString => {
  ...
  def twitterCallback(oauth_token: String, oauth_verifier: String): Future[HttpResponse] = {
    Redis
        ...
        tokenOpt
          .map(tokenMap => {
            ...
            val response: Future[HttpResponse] = oAuthResponseF.flatMap {
              ...
              case ex: Exception => {
                logger.error(
                    s"InternalServerError: Exception recovered while taking access token: $ex")
                internalServerError
              }
            }
            response
      ...
      .recover {
        case ex: Exception => {
          logger.error(
              s"InternalServerError: Exception recovered while querying Redis for $oauth_token: $ex")
          internalServerError
        }
      }
  }
        ...
        tokensOpt
          .map(tokens => {
            ...
            val request = HttpRequest(
                ...
                uri = verificationUri,
                headers =
                  Seq(Authorization(GenericHttpCredentials("OAuth", headerParamsForRequest)))
            )
  ...
  def registerTwitterUser(tokens: Map[String, String]): Future[HttpResponse] = {
    Redis
      .setHM(tokens("screen_name"), tokens)
      .map(isSuccessful => {
        if (isSuccessful) {
          ...
          twitterUserOpt
            .map(twitterUser => {
              val successMessage =
                s"${twitterUser.screen_name} is successfully registered!"
          ...
          logger.warn(
            "InternalServerError: Registering user is failed, saving user to Redis is failed")
          internalServerError
        }
      })
      .recover {
        case ex: Exception => {
          logger.error(
            s"InternalServerError: Exception recovered while saving user data to Redis: $ex")
          internalServerError
        }
      }
  }
